...
const runCodexLimitsSmoke = process.env.RUN_CODEX_LIMITS_SMOKE === "1";

test(
  "Codex limits live smoke returns a valid provider snapshot",
  { skip: !runCodexLimitsSmoke },
  async () => {
    const result = await execFileAsync(
      process.execPath,
      ["--experimental-strip-types", cliPath, "limits", "--provider", "codex", "--json"],
      {
        cwd: repoRoot,
        timeout: 20_000,
        maxBuffer: 1_000_000,
